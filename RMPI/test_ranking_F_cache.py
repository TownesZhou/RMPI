...
def get_rank(args):
    """
    Load pre-computed subgraphs from disk and compute ranks
    """
    ...
    # Together, they should be packed into a 3-tuple (en_subgraphs, dis_subgraphs, r_labels)
    #   which will be fed to the model to compute scores for the given links.

    # Check directory exists
    assert os.path.exists(os.path.join('data', params.dataset, 'neg_subgraphs')), \
        'Directory containing pre-computed subgraphs not found!'
    ...
    if head_target_id != 10000:
        ...
        data_head_path = os.path.join('data', params.dataset, 'neg_subgraphs', f'run_{run_id}_link_{neg_link_id}_head.bin')
        assert os.path.exists(data_head_path), f'File containing pre-computed subgraphs not found! Expected {data_head_path}.'
        data_head = load_graphs(data_head_path)
        # Check length
        assert len(data_head[0]) == len(head_neg_links) * 2, \
            f'Number of subgraphs loaded from {data_head_path} does not match the number of links!'
        # Batch the subgraphs
        en_subgraphs = dgl.batch(data_head[0][:len(head_neg_links)])
        dis_subgraphs = dgl.batch(data_head[0][len(head_neg_links):])
        labels = data_head[1]['labels']
        # Pack into a 3-tuple
    ...
    if tail_target_id != 10000:
        ...
        # Load pre-computed subgraphs from disk
        data_tail_path = os.path.join('data', params.dataset, 'neg_subgraphs', f'run_{run_id}_link_{neg_link_id}_tail.bin')
        assert os.path.exists(data_tail_path), f'File containing pre-computed subgraphs not found! Expected {data_tail_path}.'
        data_tail = load_graphs(data_tail_path)
        # Check length
        assert len(data_tail[0]) == len(tail_neg_links) * 2, \
            f'Number of subgraphs loaded from {data_tail_path} does not match the number of links!'
        # Batch the subgraphs
        en_subgraphs = dgl.batch(data_tail[0][:len(tail_neg_links)])
        dis_subgraphs = dgl.batch(data_tail[0][len(tail_neg_links):])
        labels = data_tail[1]['labels']
        # Pack into a 3-tuple
        data = (en_subgraphs, dis_subgraphs, labels)
